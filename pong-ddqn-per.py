...
net.compile(optimizer=Adam(1e-4), loss='mse')
...
def train():
    ...
    for ep in range(6000):
        ...
        for t in range(99000):
            ...
            if t > 0:
                old_reward = memory[-1][2]
                # Fix: Should use target_qvals here!
                old_err = np.abs(old_qval - (old_reward  + gamma*qvals[0][np.argmax(qvals[0])]))
                memory[-1][4] = old_err
            old_qval = qvals[0][ya]
            round_done = (reward != 0)
            memory.append([x, ya, reward, round_done, 0])
            reward_sum += reward
            ...
            if done:
                if ep % 3 == 0:
                    for k in range(2):
                        ...
                        sample_idxs = sorted(range(len(memory)-1), key=lambda i: memory[i][4], reverse=True)
                        sample_idxs = sample_idxs[:min(len(memory)-1, 16384)]
                ...
                if ep % 6 == 0:
                    ...
                    target_net.save('params/per_net.h5')
...
def test():
    ...
    target_net.load_weights('params/per_net.h5')
